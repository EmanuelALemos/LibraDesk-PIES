...
--2. Qual é o nome completo da pessoa com o ID 3?
SELECT CONCAT(pnome, ' ', sobrenome) AS nome_completo
FROM Pessoa
WHERE id = 3;
--3. Quais são os livros disponíveis na biblioteca (ou seja, aqueles que não foram emprestados)?
SELECT Livro.titulo
FROM Livro
LEFT JOIN Livros_Emprestimo ON Livro.id = Livros_Emprestimo.id_livro
WHERE Livros_Emprestimo.id_livro IS NULL;
--4. Quais são os empréstimos que possuem uma multa maior que 0?
SELECT *
FROM Emprestimo
WHERE multa > 0;
--5. Quais são os nomes dos leitores que possuem dois números de telefone?
SELECT Pessoa.pnome, Pessoa.sobrenome
FROM Pessoa
INNER JOIN Leitor ON Pessoa.id = Leitor.id_pessoa
WHERE Leitor.telefone_dois IS NOT NULL;
--6. Qual é o título do livro mais emprestado?
SELECT Livro.titulo
FROM Livro
INNER JOIN Livros_Emprestimo ON Livro.id = Livros_Emprestimo.id_livro
GROUP BY Livro.titulo
ORDER BY COUNT(Livros_Emprestimo.id_livro) DESC
LIMIT 1;
--7. Quais são os empréstimos realizados por um leitor com o CPF '555.555.555-55'?
SELECT *
FROM Emprestimo
WHERE cpf_leitor = '555.555.555-55';
--8. Quais são os livros que estão atualmente emprestados (ou seja, não foram devolvidos)?
SELECT Livro.titulo
FROM Livro
INNER JOIN Livros_Emprestimo ON Livro.id = Livros_Emprestimo.id_livro
INNER JOIN Emprestimo ON Livros_Emprestimo.id_emprestimo = Emprestimo.id
WHERE Emprestimo.data_devolucao > CURRENT_DATE;
--9. Qual é o valor total das multas aplicadas em todos os empréstimos?
SELECT SUM(multa) AS valor_total_multas
FROM Emprestimo;
--10. Qual é o valor total das multas aplicadas em todos os 
--empréstimos realizados por um leitor com o CPF '555.555.555-55'?
SELECT SUM(multa) AS valor_total_multas
FROM Emprestimo
WHERE cpf_leitor = '555.555.555-55';
